...
class TestSaleCommission(SingleTransactionCase):
    def setUp(self):
        ...
        self.saleorder1 = self.resource_make(
            "sale.order",
            xref="z0bug.saleorder1",
            values={
        ...
        self.saleorder3 = self.resource_make(
            "sale.order",
            xref="z0bug.saleorder3",
            values={
                "partner_id": self.partner.id,
                "order_line": [
                    (
                        ...
                        0,
                        {
                            ...
                            "agents": [
                                (
                                    ...
                                    {
                                        "agent": agent_semi.id,
                                        "commission": "z0bug.commission_net_paid",
                                    },
                                )
                            ],
                        },
        ...
        self.saleorder4 = self.resource_make(
            "sale.order",
            xref="z0bug.saleorder4",
            values={
                "partner_id": self.partner.id,
                "order_line": [
                    (
                            ...
                            "agents": [
                                (
                                    ...
                                    0,
                                    {
                                        ...
                                        "commission": "z0bug.commission_net_invoice",
                                    },
                                )
                            ],
                        },
    ...
    def tearDown(self):
        super(TestSaleCommission, self).tearDown()
    ...
    def test_sale_commission_gross_amount_payment(self):
        ...
        self.assertNotEquals(
            len(self.saleorder1.invoice_ids), 0, "Invoice should be created."
        )
        ...
        journals = self.env["account.journal"].search(
            [("type", "=", "cash"), ("company_id", "=", self.saleorder1.company_id.id)],
            limit=1,
        )
        for invoice in self.saleorder1.invoice_ids:
    ...
    def test_sale_commission_net_amount_payment(self):
        ...
        wizard.action_settle()
        journals = self.env["account.journal"].search(
            [("type", "=", "cash"), ("company_id", "=", self.saleorder3.company_id.id)],
            limit=1,
        )
        for invoice in self.saleorder3.invoice_ids:
            invoice.pay_and_reconcile(journals[:1], invoice.amount_total)
        self.assertNotEquals(
            len(self.saleorder3.invoice_ids), 0, "Invoice should be created."
        )
    ...
    def test_sale_commission_section_payment(self):
        ...
        self.assertNotEquals(
            len(self.saleorder5.invoice_ids),
            0,
            "Invoice should be created after make advance invoice where type"
            " is 'Invoice all the Sale Order'.",
        )
        ...
        journals = self.env["account.journal"].search(
            [("type", "=", "cash"), ("company_id", "=", self.saleorder5.company_id.id)],
            limit=1,
        )
        for invoice in self.saleorder5.invoice_ids:
            invoice.pay_and_reconcile(journals[:1], invoice.amount_total)
        self.assertNotEquals(
            len(self.saleorder5.invoice_ids), 0, "Invoice should be created."
        )
    ...
    def test_sale_default_agent(self):
        ...
        self.assertNotEquals(
            len(saleorder.mapped("order_line.agents")),
            0,
