...
class SaleCommissionMixin(models.AbstractModel):
    ...
    def _default_agents(self):
        ...
        if self.env.context.get("partner_id"):
            ...
            for agent in partner.agents:
                agents.append({"agent": agent.id, "commission": agent.commission.id})
    ...
    def recompute_agents(self):
        ...
        for record in self:
            ...
            record.agents = [(0, 0, vals) for vals in record._prepare_agents_vals()]
    ...
    def _get_commission_amount(
        ...
        if not commission_free and commission:
            if commission.amount_base_type == "net_amount":
                ...
                amount_base = max([0, (subtotal - (product.standard_price * quantity))])
            else:
