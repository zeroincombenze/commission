...
class Settlement(models.Model):
    ...
    agent = fields.Many2one(comodel_name="res.partner", domain="[('agent', '=', True)]")
        ...
        lang = self.env["res.lang"].search(
            [
                (
                    "code",
                    "=",
                    invoice.partner_id.lang or self.env.context.get("lang", "en_US"),
                )
            ]
        )
    ...
    def make_invoices(self, journal, product, date=False):
        ...
        for settlement in self:
            ...
            invoice_journal = (
                journal if (settlement.total + extra_total) >= 0 else False
            )
